...
2019.12.18 - charles抓包用的???咋玩的
2019.12.23 - 我还瞧不起自己写的案例==现在好了，果然还是需要的。越详细越好啊，这个东西
2019.12.25 - Nameko和celery和rabbitmq等，都是听都没有听过的东西
2019.12.31 - 整个框架流程还是要好好想想，整个体系是有点大的了
2019.12.31 - 目前管理接口是实现了，但是如何前端展示，分布式集群这个暂时就不考虑了，只用一个后台即可。task如何管理，如何展示目标的pid，汇总并提供杀死的方法呢。task如何定义呢
2019.12.31 - 优化项目：
            1、scrapy的爬虫，比如爬虫太多，则浪费大量的内存，影响机器性能，最常用的就是拆分项目，但是单个项目的爬虫也可以达到500+左右。比以前好很多但是还是膈应。单独加载一个爬虫文件，这是个不错的优化呀
            2、爬虫调度系统中，爬虫状态管理的优化，引入XX机制 -- 什么是事件机制：事件源（事件发送者）、事件（事件源发出的状态）、事件监听者。
2020.01.01 - 关于scheduler socket，是用来管理节点的，可以完善下，起码要增加一些接口，如远程关闭节点的进程，节点统计好像不太准确啊。如何关注节点是否存活呢，这是个问题。
2020.01.01 - 节点自动部署，也就是一些git clone，一些自动mkdir之类的，自动部署环境，感觉还是挺重要的啊
2020.01.06 - 分布式最难的点，是调度。感觉这回是我项目中的最大问题啊
2020.01.06 - 反向代理是一个难点啊，
2020.01.10 - 分布式调度，美观是一个问题，部署也是一个问题，应该可以部署到github上，接口由47机器提供即可。里面的爬虫也少了，多写几个也是可以的。自如，拉钩，
2020.01.10 - 接口上的说明也需要专业一点啊。太草率了吧
...
2020.01.14 - 调度系统还需要做几件事：
            1、整体页面上的优化
            2、tasks和spider需要分开，感觉也可以搞成具体的节点样式。但是这样搞的好处是什么呢  -- 感觉没什么必要啊，这种是针对不同的爬虫不同的配置来定的
            3、主要是为了好管理，如何安全的对外提供服务。  -- 这个就这样算了那
            4、task需要单独的git维护，这个还没有实现。git pull的时候需要面ssh认证。还需要在salve上面增加配置。  -- 环境还没有配置好
            5、定时回查这个东西也是一个问题。应该是挂在server端的一个服务吧，定时对node_socket进行检查。如果由于新增的，则记录最新的。
            是检查节点好还是检查每一个任务好呢。这玩意应该单独开一个服务把！  -- 没想象中写的好啊
2020.01.14 - pdfplumber 不错的pdf包
2020.01.18 - 总体来说。github上面的部署是一个问题。还有git-pull这个到底怎么免密码啊，无语，搞好还要适配下，不一定能成功
2020.01.19 - 节点清不掉，如果节点下线了，还是有一些清理不掉，看来每次点击传输局到前端的时候，需要管理下。假管理也行，不传到前端。后端的逻辑让他自己清理，这个地方需要重写吗
...
2020.01.22 - 这个假期，能做的事其实不多。但是一定要把简历写好，其中就包括对调度项目的部署，这个其实也差不多了。就是前端页面太low了。
            1、简历。目前还差个人技能不会写，这个也确实不太好写，参考的意义都不大了，水平参差不齐。
            2、部署的话，目前bug都基本解决了，但是部署这个确实有点烦人。
            3、好好准备下面试题，刷题吧。
2020.05.13 -  爬虫简历第一个项目是坑，需要尽快填好
           -  打码器，需要尽快编译出一个gif版本
           - 博客文章出大问题了，底层就没有设计好，数据结构暂时就这样? 但是绝对要分为两个模块     
"""

"""DONE!
2020.01.21 - 1、mongodb报错== MongoClient opened before fork
            2、redis数据清不掉，bug
2020.01.20 - 已修复 --- 新闻预测和房价预测也都挂了，好烦
2020.01.14 - 什么是蓝绿部署
...
2019.12.24 - 新版本项目的展示。太简单也确实没有什么意思。
            ...
            * 第一页是爬虫展示相关：自如爬虫日更新量（按地域划分的折线图）+ 区域更新（雄鸡图）+ python岗位爬虫（拉钩网+boss直招-只要岗位数量算了）+ 岗位雄鸡图
            * 第二页是新闻金融分类接口 + 房价预测。
...
2020.01.09 - 调度页面全更新着实有点过分了，如何实现首次全更新，后序点对点的修改，而且每次全更新位置都会发生变动，这是个问题
2020.01.02 - 整体来说。（如果我想要把页面部署在github，好像需要https???flask如何使用https这好像是个问题啊）
    1、数据展示部分。可以新增一些java的切换接口，使用select下拉功能，多一共一些查询的功能
    2、功能展示，这个就不说了。新闻分类done、房价预测（这个咋写呢）
    3、节点展示，基本上接口完成了，前端的展示明天应该就可以搞定，节点的存活时间感觉可以加一个。
    4、调度展示，是否可以实现那个动态滑动开关呢，这是个问题。调度还应该添加一个增加的功能，这个功能还需要设定任务的种类，烦==。
    5、日志done
    * 最后肯定是要加权限设置的，这个只能从cookie下手了。所有修改类操作都应该被拒绝。那么前端发送请求如何携带cookie呢
    * websocket聊天的也可以加一个呀，不错不错 --- 不要算了，没什么意义
    * 修改类操作执行后，能够只针对单独一个进行修改，更新，其他的就不需要操作了，感觉应该是可以的，可以参考下公司的是怎么实现的咯
    * 单独增加一个刷新按钮而不需要更新整个页面
    * 在加一个提示类的感觉就更好了呀
...
2019.12.13 - 25号深圳之旅前需要完成的项目。自如爬虫需要完成
...
2019.12.31 - multiprocessing是如何实现queue的共享的 - 数据还是在本地，只不过远程通过socket进行调用了本地的函数或者方法，并将结果传回去了而已，有点蒙
2019.12.29 - flask_apscheduler and apscheduler，这两个模块很重要啊
2019.12.26 - pywss，github上面的io可以先完成，在v2上能不能找点建议之类的

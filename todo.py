...
2019.11.25 - 降噪算法到底能不能成功实现啊。去除正弦余弦的方法到底有那些呢。
2019.11.25 - 个人站点上可以放置的。是自己的一些实践练习。如新闻分类/验证码接口/fuck, 这玩意还需好久啊。爬虫的分布。整理一下，明年。，看下全国的房价分布也不是不可以
2019.11.26 - 分布式爬虫的设计方式。首先考虑基于scrapy的问题
2019.11.29 - 把spider模仿着写一个啊，目标就是全站爬虫把。毕竟基本都有了。
2019.11.29 - 前端确实是博大精深。在线流程图还是叼的
2019.12.03 - web端的房价爬虫展示. 部署。在线爬虫管理-日志展示-爬虫编辑与开发。这个感觉可以做一个小工具啊。
2019.12.04 - asyncio的服务端可以看下, 感觉与tornado有一定的耦合性, 或许能得到些许灵感
...
2019.12.16 - excel工具看来确实需要搞一个了。针对html里面的table可以解析出文本，针对excel文件也需要可以解析出文本，word里面的table也是一样的需要解析出文本
2019.12.17 - scrapy里面是否可以使用二条说的那种，独立出爬虫对象
2019.12.18 - python后端代码是如何集群的。太神奇了吧，是否可以独立出一个单独的模块呢
2019.12.18 - charles抓包用的???
2019.12.19 - flask熟悉下，还是要通过写应用的方式啊。把自己定的标准用例写完。然后进入实战项目案例，这个可能还是要的把
"""
...
2019.08.17 - mysql数据类型有哪些，什么组电选择什么类型，类型字节大小、限制条件。索引问题。 - 打扰
...
2019.12.12 - JOBDIR这个模块也是，不支持突然中断，只能支持自然结束==都自然结束了我还需要你干哈  -- 这个就算了，也是为了安全，玩意断了，数据也会存在丢失的风险
2019.12.12 - 直接存储在FEED_URI里面 -- 存储有点僵硬，如果中途断开就断开了，完整性也不能保证 - 不能用作存储大型数据，只有爬虫正常close的时候才会存储数据，否则丢失数据
...
2019.11.29 - Flask是如何保证每一个线程对 线程本地数据 正确额push/pop的，所谓的stack...

...
* Mysql与其他主流数据库相比的特点
    * 关系型数据库
        ...
        * 联合主键：即通过多个字段唯一标记记录，
        * 外键：在某张表中，通过某字段，可以把数据与另外一张表关联起来，该字段称之为外键<br>
...
* 索引原理及其实现的机制
    ...
    * B树
    * B+树
        * 倘若仅仅使用 `SELECT * FROM cza WHERE id=1`，使用上述三种数据结构hi没有问题的<br>
        但是执行 `SELECT FROM cza WHERE id<10`，就类似全表扫描了。因为他们高度不可控<br>
        B+树高度时可控的。B+树只在最末端叶子节点存储存数据。
        * MyISAM引擎（非聚集索引）
            * 使用此引擎创建数据库表 `CREATE TABLE cza ...` 的时候，实际生成三个文件<br>
            user.myi --索引文件、user.myd --数据文件、user.frm --数据结构类<br>
            执行 `SELECT * FROM cza WHERE id=1` 执行的流程就是<br>
            1、查看该表的myi文件，有没有以id为索引的索引树<br>
            2、根据这个id索引找到叶子节点的id值，从而得到他里面的数据地址<br>
            3、根据数据地址去myd文件里面找到对应的数据返回
            * 叶子节点的data域存放的是数据记录的地址<br>
        * InnoDB索引（聚集索引）
            * 以此引擎创建数据库表，实际生成两个文件<br>
            user.ibd --索引文件、user.frm --数据结构类型<br>
            因为innodb默认以主键为索引，所以不需要myi文件。他与myisam最大的区别是将整条数据存在叶子节点。
            而不是地址。叶子节点存的是主键索引和数据信息<br>
            * 执行查询语句的时候，`SELECT * FROM cza WHERE name=cza` 执行顺序为：
            1、找到name索引树<br>
            2、根据name的值找到该树下叶子的name索引和主键值<br>
            3、用主键索引去 主键索引树，找到对应叶子节点找到对应的数据信息<br>
            * InnoDB必须要求有主键。没有则MySQL会自动选择合适的列作为索引，或者自动生成一个隐含字段作为主键<br>
            一般，请尽量使用自增字段做表的主键。因为非单调主键，在插入新数据的时候，B+树为了维持特性而频繁的分裂 <br>
            如果使用自增主键，则每次插入新数据，记录会顺序添加到当前索引节点的后续位置，当前页满则开辟新页<br>
            * 使用辅助索引，则查询会检索两遍索引，因为会先检索目标记录的主键，然后通过主键索引获取记录<br>
            为什么不推荐使用过过长的字段作为主键：<br>
            因为所有的辅助索引都需要使用主键索引，所有主键索引字段过长，会使得辅助索引变得过大。<br>

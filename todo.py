...
2020.01.14 - 调度系统还需要做几件事：
            1、整体页面上的优化
            2、tasks和spider需要分开，感觉也可以搞成具体的节点样式。但是这样搞的好处是什么呢
            3、主要是为了好管理，如何安全的对外提供服务。
            4、task需要单独的git维护，这个还没有实现。git pull的时候需要面ssh认证。还需要在salve上面增加配置。  -- 这玩意我居然写好了
            5、定时回查这个东西也是一个问题。应该是挂在server端的一个服务吧，定时对node_socket进行检查。如果由于新增的，则记录最新的。
            是检查节点好还是检查每一个任务好呢。这玩意应该单独开一个服务把！
